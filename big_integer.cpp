...
big_integer::big_integer(my_vector const &v, bool s) : data(v) {
    if (v.size() == 1 && v[0] == 0) {
        s = true;
    }
    sign = s;
...
my_vector big_integer::quotient(my_vector a, uint32_t b) {
    size_t n = a.size();
    my_vector res(n, 0);
    uint64_t carry = 0;
...
my_vector big_integer::remainder(my_vector a, uint32_t b) {
    size_t n = a.size();
    my_vector res;
    uint64_t carry = 0;
